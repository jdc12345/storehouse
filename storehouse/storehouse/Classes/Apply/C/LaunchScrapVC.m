...
- (void)viewDidLoad {
    ...
    self.itemTypeArray = [NSArray arrayWithObjects:@"申请部门",@"申请人",@"物品名称",@"报废类型",@"报废日期",@"物品数量",@"备注说明", nil];
...
- (void)addLaunchBtnClick:(UIBarButtonItem*)sender{
    ...
    if (cell6.contentField.text.integerValue  > 0) {
        totalNum = cell6.contentField.text;
    }else{
    ...
    if (!self.curruntSelectedThing) {
        ...
        assetsId = self.curruntSelectedThing.info_id;
    }
    //报废方式id
...
//http://192.168.1.168:8085/mobileapi/assetScrap/save.do?scrapModeId=1&assetsId=1&scrapDate=2018-08-17
    ...
    NSString *reportUrlStr = [NSString stringWithFormat:@"%@scrapModeId=%@&assetsId=%@&totalNum=%@&scrapDate=%@&comment=%@",mLaunchAssetScrapThing,scrapModeId,assetsId,totalNum,self.scrapDate,comment];
...
- (void)requestmStoreThingsListWith:(NSString*)searchName
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=0&limit=6",mStoreScrapThingsList,searchName];
...
-(void)refreshHeader{
    ...
    [SVProgressHUD show];
    //    http://192.168.1.168:8085/mobileapi/asset/fpRecipients.do?name=搜索内容
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=0&limit=6",mStoreScrapThingsList,self.searchField.text];
    //把搜索中文转义
...
-(void)refreshFooter{
    ...
    [SVProgressHUD show];
    //    http://192.168.1.168:8085/mobileapi/asset/fpRecipients.do?name=搜索内容
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=%ld&limit=6",mStoreScrapThingsList,self.searchField.text,(long)start];
    //把搜索中文转义
    urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [httpManager requestWithPath:urlString method:HttpRequestGet parameters:nil prepareExecute:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        [SVProgressHUD dismiss];
