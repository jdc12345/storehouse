...
- (void)addLaunchBtnClick:(UIBarButtonItem*)sender{
    ...
    if (self.selectedThingsArr.count == 0) {
        [SVProgressHUD showInfoWithStatus:@"请添加借用物品"];
        return;
    }else{
    ...
    if (tableView == self.tableView) {
        if (indexPath.section == 0) {
            ...
            if ( indexPath.row == 3 || indexPath.row == 4) {
                ...
                cell.contentField.tag = 200+indexPath.row;
            }
...
- (void)requestmStoreThingsListWith:(NSString*)searchName
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=0&limit=6",mStoreBorrowThingsList,searchName];
...
-(void)refreshHeader{
    ...
    [SVProgressHUD show];
    //    http://192.168.1.168:8085/mobileapi/asset/fpRecipients.do?name=搜索内容
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=0&limit=6",mStoreBorrowThingsList,self.searchField.text];
    //把搜索中文转义
...
-(void)refreshFooter{
    ...
    [SVProgressHUD show];
    //    http://192.168.1.168:8085/mobileapi/asset/fpRecipients.do?name=搜索内容
    NSString *urlString = [NSString stringWithFormat:@"%@name=%@&start=%ld&limit=6",mStoreBorrowThingsList,self.searchField.text,(long)start];
    //把搜索中文转义
    urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [httpManager requestWithPath:urlString method:HttpRequestGet parameters:nil prepareExecute:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        [SVProgressHUD dismiss];
