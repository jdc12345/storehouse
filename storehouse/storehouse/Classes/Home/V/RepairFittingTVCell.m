...
@end
@implementation RepairFittingTVCell
...
-(void)setStoreThingModel:(FittingsListModel *)storeThingModel{
    _storeThingModel = storeThingModel;
    self.contentField.enabled = NO;
    self.contentField.text = storeThingModel.num;
    self.assetTypeLabel.text = storeThingModel.specTyp;
...
//设置选中的数据到领用列表
-(void)setSelectedThingsModel:(FittingsListModel *)selectedThingsModel{
    _selectedThingsModel = selectedThingsModel;
    self.contentField.text = selectedThingsModel.num;
    self.assetTypeLabel.text = selectedThingsModel.specTyp;
...
- (void)setupUI{
    ...
    //assetTypeLabel资产类型
    UILabel *assetTypeLabel = [UILabel labelWithText:@"型号" andTextColor:[UIColor colorWithHexString:@"373a41"] andFontSize:12];
    ...
    [assetTypeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(emptyBtn);
        make.left.equalTo(assetNameLabel.mas_right);
        make.width.offset(100);
        make.height.offset(35);
    }];
    ...
    conentField.placeholder = @"请填写配件数量";
