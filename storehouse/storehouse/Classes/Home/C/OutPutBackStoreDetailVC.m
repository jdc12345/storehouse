...
@interface OutPutBackStoreDetailVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
@property (nonatomic, strong) NSArray *itemTypeArray;//事项名称
@property (nonatomic, strong) NSMutableArray *assetsArray;//申请相关资产列表
@property (nonatomic, strong) GetApplyDetailModel *backStoreModel;//请求的退库申请数据模型

@end

@implementation OutPutBackStoreDetailVC
...
-(void)setModel:(GetApplyDetailModel *)model{
    ...
    self.itemTypeArray = [NSArray arrayWithObjects:@"申请部门",@"申请人",@"退库理由",@"申请时间", nil];
    NSString *listUrlStr = [NSString stringWithFormat:@"%@id=%@",mAssetReturnDetail,model.info_id];
    ...
    [httpManager requestWithPath:listUrlStr method:HttpRequestPost parameters:nil prepareExecute:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([dic[@"code"] isEqualToString:@"0"]) {
            NSDictionary *getApplyDic = dic[@"assetReturn"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        switch (indexPath.row) {
            case 0:
                cell.itemContentLabel.text = self.backStoreModel.departmentName;
                break;
            case 1:
                cell.itemContentLabel.text = self.backStoreModel.userName;
                break;
            case 2:
                cell.itemContentLabel.text = self.backStoreModel.comment;
                break;
            case 3:
                cell.itemContentLabel.text = self.backStoreModel.gmtCreateString;
                break;
        ...
        cell.inboundDateString = self.backStoreModel.inboundDateString;//必须在model赋值前赋值
        cell.BackStoreModel = self.assetsArray[indexPath.row];
        if (self.backStoreModel.inboundDateString.length < 1) {
            cell.contentField.tag = [cell.BackStoreModel.info_id integerValue] + 50;//为了标记textfield查找对应cell的数据model
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (self.backStoreModel.inboundDateString.length < 1) {//未退库
        ...
        [confirmBtn addTarget:self action:@selector(confirmBackBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        return footerView;
    }
    return nil;
}
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if (self.inboundDateString.length > 1) {
        return 0;
    }else{
        if (section == 1) {
            return 150;
...
//确定按钮点击事件
-(void)confirmBackBtnClick:(UIButton*)sender{
//    出库入库》资产退库申请》退库接口、入库接口
//http://192.168.1.168:8085/mobileapi/assetReturn/saveInbound.do
//    参数：
//    |id             |Long     |Y    |编号
...
//    3=对应编号的退库申请不存在！
    ...
    [SVProgressHUD show];// 动画开始
    NSString *reportUrlStr = [NSString stringWithFormat:@"%@&id=%@&assetsIds=%@",mfpOutPutAssetBackStoreConfirmRequest,self.backStoreModel.info_id,assetsIds];
    ...
    [httpManager requestWithPath:reportUrlStr method:HttpRequestPost parameters:nil prepareExecute:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        NSDictionary *dic = (NSDictionary *)responseObject;
        if ([dic[@"code"] isEqualToString:@"0"]) {
            [SVProgressHUD showSuccessWithStatus:@"退库成功"];
